...
function menu(){
  ...
  echo ""
  echo "\e[91m¡AVISO! Se formateará todo el contenido del USB o disco que selecciones, no me hago responsable del uso que le des al Script.\e[0m"
  echo ""
...
function error_reported(){
  if [ $iso = 1>&2 ];then
     echo ""
     echo "\e[91mERROR: Coloca la imagen iso en el directorio del Script (Solo 1)\e[0m"
     exit 0
  fi
}
...
do
  case "$opt" in
    1)
      ...
      clear
      echo ""
      echo "\e[92m¡Listo!\e[0m \e[96mUSB preparado\e[0m"
      echo ""
      menu
    ;;
    2)
